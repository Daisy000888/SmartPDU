...
//mode:0,·´°×ÏÔÊ¾;1,Õý³£ÏÔÊ¾				 
//size:Ñ¡Ôñ×ÖÌå 16/12 
void OLED_ShowChar(u8 x,u8 y,u8 chr,u8 Char_Size,u8 Reverline)
{      	
...
//ÏÔÊ¾Ò»¸ö×Ö·ûºÅ´®
void OLED_ShowString(u8 x,u8 y,u8 *chr,u8 Char_Size, u8 Reverline)
{
	unsigned char j=0;
	while (chr[j]!='\0')
	{
        OLED_ShowChar(x,y,chr[j],Char_Size, Reverline);
		x+=8;
		if(x>122)
        {
            x=2;
...
//ÏÔÊ¾Ò»¸ö×Ö·ûºÅ´®
void OLED_ShowNChar(u8 x,u8 y,u8 *chr, u8 len, u8 Char_Size, u8 Reverline)
{
	unsigned char j;
...
////ÏÔÊ¾Ò»¸ö¸¡µãÊý
//u8 OLED_ShowFloatnum(u8 x, u8 y, float fnum, u8 fontsize)
//{
//    u8   len;
//    char aa[10]={0};
//    
//    len = sprintf(aa,"%.1f",fnum);//¸¡µãÐÍÊý¾Ý×ªÎªÖ¸¶¨¸ñÊ½µÄ×Ö·û´®
//    OLED_ShowString(x, y, (u8 *)aa, fontsize); //Ö±½ÓËÍ×Ö·ûÏÔÊ¾º¯Êý
//    
//    return len;
//}
...
//void OLED_ClearLine(u8 StartX,uint8_t EndX, u8 Line, u8 fontsize)
//{
//	u8 loop;
//	
//	for(loop=StartX; loop<=EndX; loop+=8)
//        OLED_ShowChar(loop, Line, ' ', fontsize);
//}
